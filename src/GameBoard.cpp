...
// called by the handle() function
// int key: the key code generated by FLTK
void GameBoard::handle_keyboard(int key) {
...
// called by the handle() function
// int ex, ey: the event x, y generated by FLTK
void GameBoard::handle_mouse(int ex, int ey) {
...
// called by the handle() function
// int ex, ey: the event x, y generated by FLTK
void GameBoard::handle_mouse_move(int ex, int ey) {
    int grid = h() / game->board().size();
    int x_ = ex - x();
    int y_ = ey - y();
    int x_grid = x_ / grid;
...
int GameBoard::handle(int event) {
    ...
    if (event == FL_KEYBOARD || event == FL_PUSH) {
        ...
        if (!game->win() && !game->lose()) {
            if (event == FL_KEYBOARD) { // KEYBOARD EVENT
                handle_keyboard(Fl::event_key());
            } else if (event == FL_PUSH) { // MOUSE CLICK
    ...
    if (event == FL_MOVE) {
        handle_mouse_move(Fl::event_x(), Fl::event_y());
